 ...
 *
 * Shape ASSUMED from issues #132/#133 (half-hourly temp/wind/precip over
 * −12h…+24h + a "now" index). Reconcile against W6a's frozen envelope is a
 * one-file change — keep the generator here, isolated from the component.
 */
...
const STEPS_PER_HOUR = 60 / INTERVAL_MINUTES; // 2
...
const pointAt = (step: number): WeatherTimelinePointV1 => {
  ...
  // Precip probability: a forecast storm window in the +3h…+9h range.
  const precipCenter = 6; // hours from now
  const precipWidth = 3;
  const bump = Math.exp(
    -((hoursFromNow - precipCenter) ** 2) / (2 * precipWidth ** 2),
  );
  const precip_pct = Math.round(Math.min(90, 5 + 85 * bump));
...
const points: readonly WeatherTimelinePointV1[] = Object.freeze(
