...
| **In progress** | Kiosk hide pointer #111 |
| **In review** | - |
...
### N5 [now-playing] BUG — album artwork missing on device · #128 ✅ Done
...
- **Done**: root cause was that no JPEG/PNG decoder existed anywhere in the host.
  ...
  Car Thing for the current track via CDP screenshot (`scripts/device-cdp.py`). PR #148
  merged, #128 closed.
...
### N6b [now-playing] Preserve queue when playing selected item · #173 ✅ Done
...
- **Scope**: correct host `play_queue_item` behavior; preserve the selected item plus cached queue
  suffix in one Spotify playback request; add focused client/service/gateway regressions.
- **Constraints**: host behavior only; frozen `{id}` intent; no device UI, input bridge, Nix,
  lyrics-provider, or generic transport changes. Blocks N7 #130 physical acceptance.
- **Acceptance**: mocked host boundary reproduces the one-item context failure; selected track and
  remaining order are preserved; host tests and required `ci` pass; branch-local Car Thing check
  selects beyond row four without filling the queue with duplicates.
- **Done**: PR #174 host/aggregate `ci` green; focused tests 42/42 and full host suite 228/228.
...
### N7 [now-playing] Interactive queue UI · #130 · ✅ Done
...
- **Done**: device-ui check green (31 files / 206 tests, typecheck, production build); PR #172
  rebased onto `origin/master` post-#175 (doc-only conflicts resolved) with required `ci` green.
  The queue-corruption defect found during initial live testing (gateway snapshot with 10 copies
  of one track) was host-side and fixed by N6b #173/PR #174; the physical re-check using PR
  #172's UI artifact against the fixed host passed (beyond-row-four press changed playback and
  the queue stayed varied — logged under N6b's Done entry and PR #174's body). PR #172 merged,
  #130 closed.

### N10 [now-playing] Keep queue coherent across track changes and playlist end · #156 · Ready
...
- **Depends**: N7 #130 merged 2026-07-25 (PR #172) — no longer blocked.
...
- **Status**: re-scoped from the original "Adaptive host poll" card (still size 2, Ready) into
  this queue-coherence card (size 5, `cross-lane`) to fold in live queue-corruption findings from
  #130/#173. Unblocked now that #130 is merged; not yet started.
...
### Drop Feed lane · #161 · Done (PR #163)
...
- **Done**: host 226 + device-ui 198 green; physical preset 3 → Photo passed; retained-screen
  preset rails corrected from `3:fd` to `3:ph`; required PR `ci` green.
...
### E3 [etymology] BUG — wire drill-down interactivity · #135 ✅ Done
...
- **Done**: PR #140 merged (CI green); physical wheel/press/back validation all-pass on device
  (blocked earlier by an unrelated on-device `input-bridge` crash-loop, since fixed — see
  `resolutions/input-bridge-crash-looping.md`).
