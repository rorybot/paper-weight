defmodule PaperWeight.Weather.JsonLite do
  @moduledoc false
  # Minimal JSON object/array decoder for NWS/OpenUV fixtures when :json is unavailable.
  # Not a full JSON library — sufficient for known weather API shapes.
  ...
  defp parse_number(bin) do
    case Regex.run(~r/^-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?/, bin) do
      [num] ->
        ...
        value =
          if String.contains?(num, ".") or String.contains?(num, "e") or String.contains?(num, "E") do
  ...
  defp normalize_float(num) do
    # String.to_float requires a decimal point
    cond do
      String.contains?(num, ".") -> num
